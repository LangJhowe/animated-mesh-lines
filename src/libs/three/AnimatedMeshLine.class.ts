...
import MeshLineGeometry from './utils/MeshLineGeometry.class'
import MeshLineMaterial from './utils/MeshLineMaterial.class'
import getRandomFloat from './utils/getRandomFloat'
export interface AnimatedMeshLineOpts {
  width: number
  speed: number
  ...
  length: number
  nbrOfPoints: number
  orientation: Vector3 // 整体方向
  turbulence: Vector3 // 自身涡流方向
  transformLineMethod?: any
}
const defaultOpts:AnimatedMeshLineOpts = {
  width: 0.1,
  speed: 0.01,
  visibleLength: 0.5,
...
export default class AnimatedMeshLine extends Mesh {
  ...
  dyingAt:number
  diedAt:number
  constructor (opts:AnimatedMeshLineOpts) {
    ...
    if (points.length == 0) {
      ...
      const vector2Points = linePoints.map((p) => {
        return new Vector2(p.x, p.y)
      })
      const curve = new SplineCurve(vector2Points)
      lineGeometry = new Geometry().setFromPoints(curve.getPoints(50))
      meshLineGeometry.setGeometry(lineGeometry, transformLineMethod)
    } else {
      meshLineGeometry.setGeometry(points, transformLineMethod)
    }
    // * ******************************
    // * Create the MeshLineGeometry

    const geometry = meshLineGeometry.geometry
